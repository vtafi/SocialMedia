...
import type { IConversation, IMessage, ChatUser } from "../../services/chat.service";
...
const getAvatarUrl = (user: ChatUser | null | undefined) => {
  if (!user) return `https://ui-avatars.com/api/?name=U&background=0052FF&color=fff`;
...
const Chat: React.FC = () => {
          ...
          .map((c) =>
            c._id === msg.conversationId
              ? {
                  ...
                  lastMessage: {
                    content: msg.content,
                    senderId: typeof msg.senderId === "string" ? msg.senderId : (msg.senderId as ChatUser)._id,
  ...
  const getOtherParticipant = (conv: IConversation): ChatUser | null => {
    ...
    if (other.userId && typeof other.userId === "object" && "_id" in other.userId) {
      return other.userId;
    }
    return {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] font-inter">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 pb-8">
        <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex overflow-hidden"
          ...
          <div className="w-80 shrink-0 border-r border-slate-100 flex flex-col">
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="font-calistoga text-2xl text-[#0F172A]">Messages</h2>
              <button
              ...
              {newChatOpen && (
                ...
                >
                  ...
                  {searchResults.map((u) => (
                    ...
                    >
                      <img src={getAvatarUrl(u)} className="w-9 h-9 rounded-full object-cover" alt={u.name} />
                      <div>
            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-slate-400 px-4 text-center">
                  ...
                  <p className="text-sm font-medium text-[#0F172A]">No conversations yet</p>
                  <p className="text-xs mt-1">Click the icon above to start chatting</p>
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="relative shrink-0">
                        <img src={getAvatarUrl(other)} className="w-11 h-11 rounded-full object-cover" alt="" />
                        {online && (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-baseline justify-between gap-1">
                          <p className={`text-sm font-semibold truncate ${isActive ? "text-[#0052FF]" : "text-[#0F172A]"}`}>
                            {other?.name ?? "Unknown"}
                          </p>
          ...
          <div className="flex-1 flex flex-col bg-[#FAFAFA] min-w-0">
            {activeConv ? (
                  ...
                  return (
                    <div className="px-6 py-4 bg-white border-b border-slate-100 flex items-center justify-between shrink-0">
                      <div className="flex items-center gap-3">
                        <button className="md:hidden mr-1 text-slate-500" onClick={() => setActiveConv(null)}>
                          <ArrowLeft className="w-5 h-5" />
                        </button>
                        <div className="relative cursor-pointer"
                          onClick={() => other?.username ? navigate(`/users/${other.username}`) : other?._id && navigate(`/users/id/${other._id}`)}>
                        ...
                        <div>
                          <p className="font-semibold text-[#0F172A] cursor-pointer hover:text-[#0052FF] transition-colors"
                            onClick={() => other?.username ? navigate(`/users/${other.username}`) : other?._id && navigate(`/users/id/${other._id}`)}>
                            {other?.name ?? "Unknown"}
                          </p>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3">
                  {msgLoading ? (
                    ...
                    messages.map((msg, i) => {
                      ...
                      const senderInfo = typeof msg.senderId === "object" ? msg.senderId as ChatUser : null;
                      const showAvatar = !isMe && (i === 0 || (typeof messages[i - 1].senderId === "string"
                        ? messages[i - 1].senderId
                        : (messages[i - 1].senderId as ChatUser)?._id) !== senderId);
                        ...
                        >
                          {!isMe && (
                            <div className="w-8 shrink-0">
                              {showAvatar && (
                                <img src={getAvatarUrl(senderInfo)} className="w-8 h-8 rounded-full object-cover" alt="" />
                              )}
                            </div>
                          )}
                          <div className={`max-w-[65%] flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                            <div className={`px-4 py-2.5 text-[15px] leading-relaxed rounded-2xl shadow-sm ${
                              isMe
                            ...
                            <div className={`flex items-center gap-1 mt-1 text-[10px] text-slate-400 ${isMe ? "flex-row-reverse" : ""}`}>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                ...
                <p className="font-semibold text-[#0F172A] mb-1">Your Messages</p>
